...
def index_docs():
    ...
    # MUST match the model you deployed in Azure AI Foundry ("text-embedding-3-large")
    try:
        logger.info("Initializing Azure OpenAI Embeddings...")
        embeddings = AzureOpenAIEmbeddings(
            azure_deployment=os.getenv("AZURE_OPENAI_EMBEDDING_DEPLOYMENT", "embedding-large"),
    ...
    if all_splits:
        ...
        BATCH_SIZE = 1  # Adjust based on your tier limits
        DELAY_BETWEEN_BATCHES = 2.0  # Seconds to wait between batches
        
        total_docs = len(all_splits)
        logger.info(f"Uploading {total_docs} chunks to Azure AI Search Index '{index_name}'...")
        logger.info(f"Batch size: {BATCH_SIZE}, Delay: {DELAY_BETWEEN_BATCHES}s")

        try:
            for i in range(0, total_docs, BATCH_SIZE):
                batch = all_splits[i : i + BATCH_SIZE]
                logger.info(f"Uploading batch {i // BATCH_SIZE + 1}/{(total_docs + BATCH_SIZE - 1) // BATCH_SIZE} ({len(batch)} docs)...")
            ...
            logger.info(f"Total chunks indexed: {total_docs}")
            logger.info("=" * 60)
        except Exception as e:
